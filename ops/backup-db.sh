...
set -euo pipefail
DIR="${BACKUP_DIR:-$HOME/janus-backups}"
...
ls -1t "$DIR"/janus-*.sql.gz 2>/dev/null | tail -n +$((KEEP + 1)) | xargs -I{} rm -f {} 2>/dev/null || true
echo "[backup] 보관 $(ls -1 "$DIR"/janus-*.sql.gz 2>/dev/null | wc -l | tr -d ' ')개"
