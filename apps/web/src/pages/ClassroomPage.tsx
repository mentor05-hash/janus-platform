            ...
            <div key={r.id} className="card">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 160 }}>
                  <b>{r.title}</b>
                  <span style={{ marginLeft: 8, fontSize: 12, fontWeight: 700, color: r.status === 'live' ? 'var(--good, #2a8a5f)' : 'var(--muted)' }}>{STATUS_LABEL[r.status] ?? r.status}</span>
                </div>
                ...
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--line)' }}>
                  ...
                  {roster.length > 0 && (
                    <ul style={{ margin: '0 0 10px', paddingLeft: 18, fontSize: 13 }}>
                      {roster.map((s) => <li key={s.id}>{s.name ?? s.studentId?.slice(0, 8) ?? '학생'} <span style={{ color: s.joined ? 'var(--good, #2a8a5f)' : 'var(--muted)' }}>· {s.joined ? '입장' : '미입장'}</span>{s.role === 'presenter' && <span style={{ color: 'var(--gold-d, #a97d24)' }}> · 발표자</span>}</li>)}
                    </ul>
                  )}
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  ...
                                  background: chosen ? 'var(--teal-50, #eef4fb)' : 'transparent',
                              ...
                              >
                                <span style={{ minWidth: 0, fontSize: 13 }}>
                                  ...
                                  {s.isHomeroom && <span style={{ color: 'var(--gold-d, #a97d24)', marginLeft: 6, fontSize: 12 }}>담임</span>}
                                </span>
                  ...
                  {picked.length > 0 && (
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginTop: 8 }}>
                      {picked.map((s) => (
                        <span key={s.studentId} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, background: 'var(--teal-50, #eef4fb)', color: 'var(--ink)', border: '1px solid var(--line)', borderRadius: 999, padding: '3px 6px 3px 10px' }}>
                          {s.name}
