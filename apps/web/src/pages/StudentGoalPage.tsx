...
export function StudentGoalPage() {
  ...
  return (
    <div>
      <PageHeader title="목표 설정" sub="목표 대학·학과·평균을 정하면 약점 과목이 '할 일'로 자동 제안되고, 격차 리포트가 목표 기준으로 계산돼요." />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16, alignItems: 'start' }}>
          <Card title="내 목표">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <TextField label="목표 대학" placeholder="예) 성균관대" value={goal.university ?? ''} onChange={(e) => set({ university: e.target.value })} />
              <TextField label="목표 학과" placeholder="예) 전자공학" value={goal.department ?? ''} onChange={(e) => set({ department: e.target.value })} />
              <SelectField label="목표 라인(선택)" options={TIER_OPTIONS} value={goal.tier ?? ''} onChange={(e) => set({ tier: e.target.value || null })} />
              <TextField label="목표 평균 점수" type="number" min={0} max={100} placeholder="예) 90" hint="격차 리포트의 과목별 격차 계산 기준이에요." value={goal.avg ?? ''} onChange={(e) => set({ avg: e.target.value === '' ? null : Number(e.target.value) })} />
          ...
          <Card title="목표는 이렇게 쓰여요">
            <ul style={{ margin: 0, paddingLeft: 18, fontSize: 14, color: 'var(--ink-body)', lineHeight: 1.7 }}>
              ...
              <li><b>격차 리포트</b> — 목표 대학·학과가 리포트의 목표 컷 기준으로 채워져요.</li>
              <li><b>상담·컨설팅</b> — 선생님·컨설턴트가 같은 목표를 보고 전략을 잡아요.</li>
              <li>목표 평균을 비워 두면 격차 수치 대신 목표 설정 안내가 표시돼요.</li>
            </ul>
