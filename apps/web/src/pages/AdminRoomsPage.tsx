  ...
  const columns: Column<Room>[] = [
    ...
    { key: 'setting', header: '설정', render: (r) => (
      <button onClick={() => saveRoom(r.id, { setting: r.setting === 'manual' ? 'auto' : 'manual' })}
        style={{ cursor: 'pointer', border: '1px solid var(--line)', borderRadius: 7, padding: '3px 9px', fontSize: 12, fontWeight: 700, background: r.setting === 'manual' ? 'var(--teal-50,#e0f5f7)' : '#fff', color: r.setting === 'manual' ? 'var(--teal)' : 'var(--muted)' }}>
