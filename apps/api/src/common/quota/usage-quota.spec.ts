...
describe('UsageQuota (유료 API 일 호출 상한)', () => {
  it('상한까지는 통과하고 초과분부터 던진다', async () => {
    ...
    await q.consume('ocr', 3);
    await q.consume('ocr', 3);
    await expect(q.consume('ocr', 3)).rejects.toBeInstanceOf(QuotaExceededError);
  });

  it('스코프별로 따로 센다 — 한 용도가 다 써도 다른 용도는 살아있다', async () => {
    const q = new UsageQuota(new MemoryCacheProvider(), 'llm');
    await q.consume('ocr', 1);
    await expect(q.consume('ocr', 1)).rejects.toBeInstanceOf(QuotaExceededError);
  ...
  it('질문당 1회 제한 — 같은 scope 키로는 두 번째가 막힌다', async () => {
    ...
    await expect(q.consume('draft:question-abc', 1)).rejects.toBeInstanceOf(QuotaExceededError);
    // 다른 질문은 영향 없음
  ...
  it('카운터 장애 시 기본은 차단(fail closed) — 사용량 초과가 아님을 구분해 알린다', async () => {
    ...
    await expect(q.consume('ocr', 10)).rejects.toMatchObject({ counterUnavailable: true });
  });
