...
export class DiagnosticService {
  ...
  private assertStudent(user: AuthUser) {
    if (user.role !== AccountRole.STUDENT)
      throw new ForbiddenException('학생만 진단을 볼 수 있습니다.');
  }
  ...
  async start(
    user: AuthUser,
    subject?: string,
    count = DiagnosticService.DEFAULT_COUNT,
  ) {
    this.assertStudent(user);
    const where = { active: true, ...(subject ? { subject } : {}) };
    const pool = await this.prisma.diagnostic_question.findMany({
      where,
      take: Math.min(30, Math.max(1, count)),
    });
    if (pool.length === 0)
      throw new NotFoundException('출제할 문항이 없습니다(문제은행 준비 중).');
  ...
  async adminCreateQuestion(dto: {
    ...
    if (dto.answer < 0 || dto.answer >= dto.choices.length)
      throw new BadRequestException('정답 인덱스가 보기 범위를 벗어났습니다.');
  ...
  async adminSetActive(id: string, active: boolean) {
    await this.prisma.diagnostic_question.update({
      where: { id },
      data: { active },
    });
    return { id, active };
  ...
  async startClinic(
    ...
    if (units.length === 0)
      throw new NotFoundException('클리닉할 유형이 없습니다.');
    const pool = await this.prisma.diagnostic_question.findMany({
      where: { active: true, unit: { in: units } },
      take: Math.min(30, Math.max(1, count)),
    });
    if (pool.length === 0)
      throw new NotFoundException('해당 유형 문항이 없습니다.');
    const attempt = await this.prisma.diagnostic_attempt.create({
      data: {
        student_id: user.id,
        subject: '약점클리닉',
  ...
  async submit(
    ...
    this.assertStudent(user);
    const attempt = await this.prisma.diagnostic_attempt.findUnique({
      where: { id: attemptId },
    });
    if (!attempt || attempt.student_id !== user.id)
      throw new NotFoundException('진단 시도를 찾을 수 없습니다.');
    if (attempt.submitted_at)
      throw new BadRequestException('이미 제출된 진단입니다.');
    const qids = answers.map((a) => a.questionId);
    const qs = await this.prisma.diagnostic_question.findMany({
      where: { id: { in: qids } },
    });
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.diagnostic_attempt.update({
        where: { id: attemptId },
        data: { total, correct, score, submitted_at: new Date() },
      }),
    ]);
