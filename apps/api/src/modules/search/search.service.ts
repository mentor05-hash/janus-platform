...
export type SearchResult = { q: string; total: number; groups: Record<string, SearchHit[]> };
...
export class SearchService {
  ...
  async searchAll(user: AuthUser, q: string): Promise<SearchResult> {
    ...
    const [lectures, materialsRes, community, teachers] = await Promise.all([
      ...
      this.material.list(user, { q: term }).catch(() => ({ data: [] as Array<Record<string, unknown>> })),
      this.qna.listCommunity(user, { q: term }).catch(() => []),
      this.people.listTeachers({ q: term, page: 1, size: N } as never, user).catch(() => ({ data: [] as Array<Record<string, unknown>> })),
    ]);
    const materials = (materialsRes as { data?: Array<Record<string, unknown>> }).data ?? [];
    ...
    const total = lectureHits.length + materialHits.length + communityHits.length + teacherHits.length;
