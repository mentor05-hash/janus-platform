...
export class RealtimeGateway implements OnGatewayConnection {
  ...
  private readonly windows = new Map<
    string,
    { restricted: boolean; opensMs: number; closesMs: number }
  >();
  ...
  handleConnection(client: Socket) {
    try {
      ...
      void client.join(`user:${user.id}`); // 수명주기 훅은 동기 — 개인 룸 참여는 대기하지 않는다
    } catch {
  ...
  chatTyping(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    { bookingId, typing }: { bookingId: string; typing: boolean },
  ) {
    const user = this.user(client);
    client
      .to(`booking:${bookingId}`)
      .emit('chat:typing', { bookingId, userId: user.id, typing: !!typing });
    return { ok: true };
  }
