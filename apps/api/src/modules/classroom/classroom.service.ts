...
export class ClassroomService {
  ...
  async create(dto: CreateClassDto, user: AuthUser) {
    ...
    if (this.rooms.enabled) {
      ...
      const created = await this.rooms.createRoom({
        ...
        mode: 'lecture',
        participants: [
          { extUserId: user.id, displayName: teacherName, role: 'host' },
        ],
      });
      const hostPid =
        created.participants.find((p) => p.extUserId === user.id)
          ?.participantId ??
        created.participants[0]?.participantId ??
        null;
      await this.prisma.class_session.update({
        where: { id: row.id },
        data: { room_id: created.roomId, host_participant_id: hostPid },
      });
  ...
  async enroll(classId: string, dto: EnrollDto, user: AuthUser) {
    ...
    const data = dto.studentIds.map((sid) => ({
      class_session_id: cls.id,
      student_id: sid,
      role: 'viewer',
    }));
    const res = await this.prisma.class_enrollment.createMany({
      data,
      skipDuplicates: true,
    });
    const total = await this.prisma.class_enrollment.count({
      where: { class_session_id: cls.id },
    });
  ...
  async join(classId: string, user: AuthUser) {
    const cls = await this.prisma.class_session.findUnique({
      where: { id: classId },
    });
    if (!cls) throw new NotFoundException('강의를 찾을 수 없습니다.');
    if (!this.rooms.enabled)
      throw new BadRequestException('실시간 룸 서비스가 설정되지 않았습니다.');
    if (!cls.room_id)
      throw new ConflictException('강의실이 아직 준비되지 않았습니다.');

    // 선생님(host)
    if (cls.teacher_id === user.id || this.isStaff(user)) {
      if (!cls.host_participant_id)
        throw new ConflictException('host 참가자가 없습니다.');
      const token = await this.rooms.mintToken(
        cls.room_id,
        cls.host_participant_id,
      );
      return {
    ...
    // 학생(viewer)
    const enr = await this.prisma.class_enrollment.findFirst({
      where: { class_session_id: classId, student_id: user.id },
    });
    ...
    const added = await this.rooms.addParticipant(cls.room_id, {
      extUserId: user.id,
      displayName: name,
      role: enr.role,
    });
    await this.prisma.class_enrollment.update({
      where: { id: enr.id },
      data: { participant_id: added.participantId, joined_at: new Date() },
    });
    return {
      url: this.rooms.publicUrl,
      token: added.token,
  ...
  async start(classId: string, user: AuthUser) {
    return this.transition(classId, user, 'live');
  }
  async end(classId: string, user: AuthUser) {
    return this.transition(classId, user, 'ended');
  }
  ...
  private async transition(classId: string, user: AuthUser, to: ClassStatus) {
    ...
    if (!canTransition(cls.status as ClassStatus, to)) {
      throw new ConflictException(
        `현재 상태(${cls.status})에서 ${to} 로 전환할 수 없습니다.`,
      );
    }
    const res = await this.prisma.class_session.updateMany({
      where: { id: classId, status: cls.status },
  ...
  async list(user: AuthUser) {
    ...
    const rows = await this.prisma.class_session.findMany({
      where,
  ...
  async getOne(classId: string, user: AuthUser) {
    ...
    const enrolledCount = await this.prisma.class_enrollment.count({
    ...
    if (!isOwner) {
      ...
      if (!enr)
        throw new ForbiddenException('이 강의에 접근할 권한이 없습니다.');
    }
  ...
  async mediaToken(classId: string, user: AuthUser) {
    ...
    if (!isOwner) {
      ...
      if (!enr)
        throw new ForbiddenException('이 강의에 등록되어 있지 않습니다.');
    }
    const name = await this.accountName(user.id);
    return this.media.issueToken(
      cls.room_id,
      user.id,
      isOwner ? 'publisher' : 'subscriber',
      name,
    );
  }
  ...
  async startRecording(classId: string, user: AuthUser) {
    const cls = await this.mustOwn(classId, user);
    if (!cls.room_id)
      throw new ConflictException('강의실이 준비되지 않았습니다.');
    const open = await this.prisma.class_recording.findFirst({
      where: { class_session_id: classId, status: 'recording' },
    });
    ...
    const row = await this.prisma.class_recording.create({
      data: {
        ...
        recording_ref: rec.recordingRef,
        status: 'recording',
      },
    });
  ...
  async stopRecording(classId: string, recordingId: string, user: AuthUser) {
    const cls = await this.mustOwn(classId, user);
    const rec = await this.prisma.class_recording.findFirst({
      where: { id: recordingId, class_session_id: classId },
    });
    if (!rec) throw new NotFoundException('녹화를 찾을 수 없습니다.');
    if (rec.status !== 'recording') return this.toRecordingDto(rec);
    const res =
      cls.room_id && rec.recording_ref
        ? await this.media.stopRecording(cls.room_id, rec.recording_ref)
        : { url: null };
    const row = await this.prisma.class_recording.update({
      ...
      data: {
        status: 'done',
        url: res.url ?? null,
        duration_sec: res.durationSec ?? null,
        ended_at: new Date(),
      },
    });
    return this.toRecordingDto(row);
  ...
  async listRecordings(classId: string, user: AuthUser) {
    ...
    const rows = await this.prisma.class_recording.findMany({
      where: { class_session_id: classId },
  ...
  async roster(classId: string, user: AuthUser) {
    const cls = await this.mustOwn(classId, user);
    const enrs = await this.prisma.class_enrollment.findMany({
      where: { class_session_id: cls.id },
    ...
    return {
      data: enrs.map((e) => ({
        ...
        name: e.student_id ? (nameById.get(e.student_id) ?? null) : null,
        role: e.role,
        joined: !!e.joined_at,
      ...
      meta: {
        total: enrs.length,
        joined: enrs.filter((e) => e.joined_at).length,
      },
    };
  }
  ...
  private async mustOwn(classId: string, user: AuthUser) {
    ...
    if (!cls) throw new NotFoundException('강의를 찾을 수 없습니다.');
    if (cls.teacher_id !== user.id && !this.isStaff(user))
      throw new ForbiddenException('강의 개설자만 할 수 있습니다.');
    return cls;
  }
