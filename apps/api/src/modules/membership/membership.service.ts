...
export class MembershipService {
  ...
  listPlans() {
    return this.prisma.subscription_plan.findMany({
      include: { membership_grade: { select: { name: true, weekly_credits: true, tier: true } } },
    });
  }
  ...
  async subscribe(student: AuthUser, planId: string, now = new Date()) {
    ...
    if (!plan.grade_id) throw new BadRequestException('플랜에 등급이 연결되어 있지 않습니다.');
    ...
    const nextBilling = computeNextBilling(plan.billing_cycle as BillingCycle, now);
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.student_profile.update({
        ...
        data: { membership_grade_id: plan.grade_id, active_subscription_id: sub.id },
      });
      return { subscriptionId: sub.id, planId, gradeId: plan.grade_id, nextBillingAt: nextBilling };
    });
  }
}
