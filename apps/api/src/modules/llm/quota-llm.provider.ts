...
export class QuotaLlmProvider implements LlmProvider {
  ...
  /** Q&A AI 초안 — 질문마다 걸려 호출 수가 많다(실행계획서가 지목한 폭주 지점). */
  ...
  /** 관문 자유서술 해석 — 무료 진입점이라 유입이 곧 비용이다. 가장 보수적 상한. */
  interpretGateway(input: GatewayInterpretInput): Promise<GatewayLlmResult> {
    return this.guard('gateway', () => this.inner.interpretGateway(input));
  }

  consultSummary(input: ConsultSummaryInput): Promise<ConsultSummaryResult> {
    return this.guard('consultReport', () => this.inner.consultSummary(input));
  }

  consultReportViews(
    input: ConsultReportViewsInput,
  ): Promise<ConsultReportViewsResult> {
    return this.guard('consultReport', () =>
      this.inner.consultReportViews(input),
    );
  }
