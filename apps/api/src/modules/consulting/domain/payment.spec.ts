...
describe('결제 금액 산정(원화 · 크레딧 불가)', () => {
  it('고정가 상품은 기본가 사용', () => {
    expect(resolvePaymentAmount('single')).toEqual({
      ok: true,
      amount: 150000,
    });
    expect(resolvePaymentAmount('season')).toEqual({
      ok: true,
      amount: 480000,
    });
  });
  it('고정가 상품도 override(할인) 허용', () => {
    expect(resolvePaymentAmount('single', 120000)).toEqual({
      ok: true,
      amount: 120000,
    });
  });
  it('종합 전담(맞춤 견적)은 금액 지정 필수', () => {
    ...
    expect(resolvePaymentAmount('full', 1200000)).toEqual({
      ok: true,
      amount: 1200000,
    });
  });
