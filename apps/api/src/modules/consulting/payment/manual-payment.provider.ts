...
import { ChargeInput, PaymentProvider, PaymentProviderStatus } from './payment.types';
...
export class ManualPaymentProvider implements PaymentProvider {
  ...
  async createCharge(input: ChargeInput): Promise<{ providerRef: string }> {
    ...
    this.logger.log(`createCharge app=${input.applicationId} ${input.amountWon}원 → ${providerRef}`);
    return { providerRef };
  }
  ...
  async refund(providerRef: string, amountWon: number): Promise<PaymentProviderStatus> {
