  ...
  async teacherInbox(user: AuthUser) {
    ...
    const [requests, questions, notis, unreadRows] = await Promise.all([
      ...
      this.prisma.qna_post.findMany({
        where: {
          status: 'open',
          OR: [{ assigned_teacher_id: uid }, { scope: 'open' as never }],
        },
        select: {
          id: true,
          student_id: true,
    ...
    const unreadChats = Object.values(unreadByBooking).reduce(
      (a, b) => a + b,
      0,
    );
    ...
    return {
      ...
      requests: requests.map((r) => ({
        ...
        consultType: r.consult_type
          ? consultTypeFromPrisma(r.consult_type)
          : null,
        subType: r.sub_type,
        mode: r.mode,
        start: r.start_at,
      ...
      questions: questions.map((q) => ({
        id: q.id,
        studentName: names[q.student_id] ?? '학생',
        body: q.body,
        assigned: q.assigned_teacher_id === uid,
