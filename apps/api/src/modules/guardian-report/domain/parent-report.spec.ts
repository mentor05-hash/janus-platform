...
const base: ParentReportInput = {
  ...
  consultations: [{ at: '2026-07-13T00:00:00.000Z', teacher: '김선생', summary: '수학 오답 패턴 점검' }],
  qnaCount: 2,
};

describe('buildParentReport (학부모 주간 통합 리포트 v1)', () => {
  it('출석률 = 완료/(완료+노쇼)', () => {
    expect(buildParentReport({ ...base, sessions: { done: 3, upcoming: 0, noshow: 1, cancelled: 0 } }).sections.attendance.rate).toBe(75);
    expect(buildParentReport(base).sections.attendance.rate).toBe(100); // 노쇼 0
  });

  it('대상 세션(완료+노쇼) 0 이면 rate null·문구', () => {
    const r = buildParentReport({ ...base, sessions: { done: 0, upcoming: 2, noshow: 0, cancelled: 0 } });
  ...
  it('활동 전무 시 안내 문구', () => {
    const r = buildParentReport({ studentName: '무활동', periodDays: 7, score: null, sessions: { done: 0, upcoming: 0, noshow: 0, cancelled: 0 }, consultations: [], qnaCount: 0 });
    expect(r.headline).toContain('활동은 없었습니다');
    expect(r.sections.score).toBeNull();
  });

  it('상담 recent 는 최대 3건, 성적 라벨 생성', () => {
    const many = Array.from({ length: 5 }, (_, i) => ({ at: `2026-07-1${i}T00:00:00Z`, summary: `s${i}` }));
