...
import { buildParentReport, type ParentReport, type ParentReportConsult, type ParentReportSessions } from './domain/parent-report';
...
export class GuardianReportService {
  ...
  async report(guardian: AuthUser, studentId: string, days = 7): Promise<ParentReport> {
    await this.assertLinked(guardian, studentId);
    const account = await this.prisma.account.findUnique({ where: { id: studentId }, select: { name: true } });
    ...
    const score = js ? { gye: js.gye, mode: js.mode, nb: js.nb, period: js.period } : null;
    ...
    const sessions: ParentReportSessions = { done: 0, upcoming: 0, noshow: 0, cancelled: 0 };
    for (const b of bookings) {
      const future = b.start_at != null && b.start_at > now;
      if (b.status === 'done') sessions.done += 1;
      else if (b.status === 'noshow') sessions.noshow += 1;
      else if (b.status === 'cancelled' || b.status === 'rejected') sessions.cancelled += 1;
      else if ((b.status === 'new' || b.status === 'confirmed') && future) sessions.upcoming += 1;
    }
    ...
    const teachers = teacherIds.length
      ? await this.prisma.account.findMany({ where: { id: { in: teacherIds } }, select: { id: true, name: true } })
      : [];
    ...
    // Q&A 활동(작성 질문 수)
    const qnaCount = await this.prisma.qna_post.count({ where: { student_id: studentId, created_at: { gte: since } } });

    return buildParentReport({ studentName: account.name, periodDays: days, score, sessions, consultations, qnaCount });
  }
}
