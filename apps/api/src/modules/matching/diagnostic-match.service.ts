...
export class DiagnosticMatchService {
  ...
  private async targetSubjects(studentId: string, subject?: string): Promise<string[]> {
    if (subject) return [subject];
    const attempt = await this.prisma.diagnostic_attempt.findFirst({
      where: { student_id: studentId, subject: { not: null }, submitted_at: { not: null } },
      orderBy: { started_at: 'desc' }, select: { subject: true },
    });
    if (attempt?.subject && attempt.subject !== '약점클리닉') return [attempt.subject];
    const report = await this.prisma.score_report.findFirst({
      where: { student_id: studentId }, orderBy: { period: 'desc' }, include: { items: { select: { subject: true } } },
    });
    const subs = [...new Set((report?.items ?? []).map((i) => i.subject))].filter(Boolean);
  ...
  async recommend(user: AuthUser, opts: { subject?: string; mode?: string; limit?: number }) {
    ...
    const scored = cands.map((c) => {
      ...
      const hit = targets.length ? targets.filter((s) => subs.includes(s) || (s === '탐구' && subs.some((x) => ['과학', '사회'].includes(x)))).length : 0;
    ...
    if (picks.length < limit) {
      ...
      const news = rest.filter((s) => s.isNew).slice(0, DiagnosticMatchService.COLD_START_SLOTS);
      const fill = rest.filter((s) => !news.includes(s)).slice(0, limit - picks.length - news.length);
      picks = [...picks, ...news, ...fill].slice(0, limit);
    }

    // 계측 — 노출(shown). studentId·과목·후보 id 만(성적 원자료 복제 금지).
    void this.recordFunnel('shown', { studentId: user.id, subjects: targets, counselorIds: picks.map((p) => p.c.account_id) });
  ...
  private async recordFunnel(event: string, meta: Record<string, unknown>) {
    try { await this.prisma.funnel_event.create({ data: { page: 'diag_match', event, cta: event, meta: meta as object } }); }
    catch { /* 계측 실패 비차단 */ }
  }
}
