...
export class SsoService {
  ...
  async issue(user: AuthUser, serviceId: string) {
    ...
    const token = signSsoToken(
      { sub: user.id, role: user.role as SsoRole, tier, aud: svc.id, scope, epoch: svc.epoch },
      this.ttlSec,
      this.secret,
    );
    const origins = (svc.redirect_origins as string[]) ?? [];
    await this.audit.record(user, { action: 'sso.issue', targetType: 'sso_service', targetId: svc.id, summary: `SSO 발급(${svc.id}·${tier})` });
    return {
      token,
      expiresInSec: this.ttlSec,
      // 오픈 리다이렉트 방지: 등록 origin 이 있을 때만 진입 URL 구성(설계 §3)
      url: origins.length ? `${origins[0].replace(/\/$/, '')}/?sso=${encodeURIComponent(token)}` : null,
    };
  }
  ...
  async verify(token: string, serviceId?: string) {
    ...
    const svc = await this.prisma.sso_service.findUnique({ where: { id: aud } });
    if (!svc) return { ok: false as const, reason: 'service_not_found' as const };
    const r = verifySsoToken(token, this.secret, svc.epoch, svc.id);
    if (!r.ok) return { ok: false as const, reason: r.reason };
    return { ok: true as const, sub: r.payload.sub, role: r.payload.role, tier: r.payload.tier, scope: r.payload.scope, service: svc.id, exp: r.payload.exp };
  }
  ...
  async upsertService(actor: AuthUser, dto: { id: string; name: string; allowedScopes?: string[]; minTier?: string; redirectOrigins?: string[] }) {
    const data = {
      name: dto.name,
    ...
    const svc = await this.prisma.sso_service.upsert({ where: { id: dto.id }, create: { id: dto.id, ...data }, update: data });
    await this.audit.record(actor, { action: 'sso.service.upsert', targetType: 'sso_service', targetId: svc.id, summary: `SSO 서비스 등록/수정(${svc.id})`, meta: dto });
    return svc;
  }

  /** 관리자: epoch 증가 = 해당 서비스 기존 토큰 전량 무효(유출 대응, 설계 §2). */
  async revokeAll(actor: AuthUser, serviceId: string) {
    const svc = await this.prisma.sso_service.update({ where: { id: serviceId }, data: { epoch: { increment: 1 }, updated_at: new Date() } });
    await this.audit.record(actor, { action: 'sso.revoke_all', targetType: 'sso_service', targetId: svc.id, summary: `SSO 일괄 폐기(${svc.id} → epoch ${svc.epoch})` });
