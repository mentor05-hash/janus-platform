...
export class RoomsBridgeService {
  ...
  private get publicUrl() {
    return this.config.get<string>('ROOMS_PUBLIC_URL') || '';
  }
  ...
  async session(user: AuthUser, bookingId: string) {
    ...
    const isStudent = b.student_id === user.id,
      isTeacher = b.teacher_id === user.id;
    if (!isStudent && !isTeacher) return { enabled: false as const };

    const access = await this.realtime.featureAccess(
      user,
    ...
    const metadata =
      b.mode === 'chat' && b.end_at
        ? {
            endAt: b.end_at.toISOString(),
            postFree: {
              limit: this.realtime.postFreeLimit,
              roles: ['student'],
            },
          }
        : undefined;
    ...
    if (existed)
      await this.rooms
        .updateWindow(map.room_id, { opensAt, closesAt, metadata })
        .catch(() => {});
    const participantId = isStudent
      ? map.student_participant_id
      : map.teacher_participant_id;
  ...
  private async ensureRoom(
    ...
    const sp =
      created.participants.find((p) => p.extUserId === studentId)
        ?.participantId ?? created.participants[0].participantId;
    const tp =
      created.participants.find((p) => p.extUserId === teacherId)
        ?.participantId ?? created.participants[1].participantId;
