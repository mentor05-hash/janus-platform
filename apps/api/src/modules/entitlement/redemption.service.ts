...
export class RedemptionService {
  ...
  async generate(
    ...
    if (!isProductKey(dto.productKey))
      throw new BadRequestException({
        code: 'PRODUCT_UNKNOWN',
        message: `알 수 없는 상품: ${dto.productKey}`,
      });
    ...
    for (let i = 0; i < count; i++) {
      rows.push({
        code: randomCode(),
        product_key: dto.productKey,
        grant_expires_at: dto.grantExpiresAt ?? null,
        valid_until: dto.validUntil ?? null,
        note: dto.note ?? null,
        created_by: actor.id,
      });
    }
    const res = await this.prisma.redemption_code.createMany({
      data: rows,
      skipDuplicates: true,
    });
    await this.audit.record(actor, {
      ...
      summary: `수강권 발급 ${res.count}개(${productLabel(dto.productKey)})`,
      meta: { productKey: dto.productKey, count: res.count },
    });
    return {
      generated: res.count,
      codes: rows.slice(0, res.count).map((r) => r.code),
    };
  }
  ...
  async redeem(user: AuthUser, rawCode: string) {
    ...
    const row = await this.prisma.redemption_code.findUnique({
      where: { code },
    });
    if (!row)
    ...
    await this.entitlement.grant(user, user.id, row.product_key, {
    ...
    await this.audit.record(user, {
      ...
      targetType: 'redemption_code',
      targetId: row.id,
      summary: `수강권 등록(${productLabel(row.product_key)})`,
      meta: { code, productKey: row.product_key },
    });
    return {
      ok: true,
