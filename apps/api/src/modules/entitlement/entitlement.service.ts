...
export class EntitlementService {
  ...
  async resolveAccount(
    ref: string,
  ): Promise<{ id: string; login_id: string; name: string; role: string }> {
    const r = (ref ?? '').trim();
    const acc = UUID_RE.test(r)
      ? await this.prisma.account.findUnique({
          where: { id: r },
          select: { id: true, login_id: true, name: true, role: true },
        })
      : await this.prisma.account.findUnique({
          where: { login_id: r },
          select: { id: true, login_id: true, name: true, role: true },
  ...
  async myEntitlements(accountId: string) {
    ...
    for (const r of rows) {
      ...
      if (g) g.services.push(r.service_id);
        ...
        groups.set(key, {
          ...
          label: productLabel(r.product_key),
    ...
    const cards = [...groups.values()].map((g) => ({
      ...g,
      daysRemaining: g.expiresAt
        ? Math.ceil((g.expiresAt.getTime() - now) / DAY_MS)
        : null,
  ...
  async runExpiryCheck(
    withinDays = 7,
  ): Promise<{ notified: number; accounts: number }> {
    const now = new Date();
    ...
    const byAcc = new Map<
      string,
      { minDays: number; label: string; ids: string[] }
    >();
    for (const r of due) {
      const days = Math.ceil(
        ((r.expires_at as Date).getTime() - now.getTime()) / DAY_MS,
      );
      ...
      if (cur) {
        ...
        if (days < cur.minDays) {
          cur.minDays = days;
          cur.label = productLabel(r.product_key);
        }
      } else
        byAcc.set(r.account_id, {
          minDays: days,
          label: productLabel(r.product_key),
          ids: [r.id],
        });
    ...
    for (const [accountId, info] of byAcc) {
      await this.notify.notify(accountId, 'entitlement_expiring', {
        days: info.minDays,
        product: info.label,
      });
    }
   ...
   */
  async syncSubscriptionProducts(
    accountId: string,
    productKeys: unknown,
    actorId?: string,
    ...
    const keys = Array.isArray(productKeys)
      ? productKeys.filter(
          (k): k is string => typeof k === 'string' && isProductKey(k),
        )
      : [];
  ...
  async revoke(actor: AuthUser, entitlementId: string) {
    const row = await this.prisma.service_entitlement.findUnique({
    ...
    const updated = await this.prisma.service_entitlement.update({
      where: { id: entitlementId },
      data: { revoked_at: new Date() },
    });
    await this.audit.record(actor, {
