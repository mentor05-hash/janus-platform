...
export class GuardianService {
  ...
  async listChildren(guardian: AuthUser) {
    ...
    const children = await Promise.all(
      links.map(async (l) => {
        ...
        return { linkId: l.id, studentId: l.student_id, name: acc?.name ?? null, relation: l.relation };
      }),
    );
    return children;
  }
  ...
  async respondLink(linkId: string, dto: GuardianLinkRespondDto, actor: AuthUser) {
    ...
    const isStudentOwner = actor.role === AccountRole.STUDENT && actor.id === link.student_id;
    ...
    const to: GuardianLinkStatus =
      dto.action === 'approve' ? 'approved' : dto.action === 'reject' ? 'rejected' : 'revoked';
    if (!canLinkTransition(link.status as GuardianLinkStatus, to)) {
      throw new BadRequestException(`허용되지 않는 연결 상태 전이: ${link.status} → ${to}`);
    }
