...
export class MemberTypeService {
  ...
  async create(dto: CreateMemberTypeDto) {
    const exists = await this.prisma.member_type.findUnique({
      where: { kind_code: { kind: dto.kind, code: dto.code } },
    });
    if (exists)
      throw new BadRequestException(
        `이미 존재하는 분류입니다(${dto.kind}/${dto.code}).`,
      );
  ...
  async deactivate(id: string) {
    ...
    return this.prisma.member_type.update({
      where: { id },
      data: { active: false },
  ...
  async assign(
    ...
    accountId: string,
    dto: SetMemberTypeDto,
    actor: AuthUser,
  ) {
    const mt = await this.prisma.member_type.findUnique({
      where: { kind_code: { kind, code: dto.typeCode } },
    });
    if (!mt || !mt.active)
      throw new BadRequestException('유효하지 않은 분류 코드입니다.');

    if (kind === 'teacher') {
      const prof = await this.prisma.teacher_profile.findUnique({
      ...
      await this.prisma.teacher_profile.update({
        where: { account_id: accountId },
        data: { type_code: dto.typeCode },
      });
    } else {
      const prof = await this.prisma.student_profile.findUnique({
        where: { account_id: accountId },
      });
