...
export const withinDailyLimit = (
  usedToday: number,
  limit = COMMUNITY_DAILY_LIMIT,
): boolean => usedToday < limit;

export type AnswerGate =
  | { allowed: true }
  | {
      allowed: false;
      reason: 'not_community' | 'hidden' | 'resolved' | 'owner';
    };

/** 커뮤니티 답변 가능 여부 — 커뮤니티 질문·미숨김·미해결·본인 질문 아님이면 누구나(전원 답변). */
export function canAnswerCommunity(p: {
  community: boolean;
  hidden: boolean;
  status: string;
  ownerId: string;
...
): boolean => reportCount >= threshold;

/** AI 초안과 유사도가 임계 이상이면 "AI 생성 미표기" 경고(사람이 초안을 그대로 붙인 정황). */
export const aiUnlabeled = (
  similarityToDraft: number,
  threshold = AI_SIMILAR_THRESHOLD,
): boolean => similarityToDraft >= threshold;
