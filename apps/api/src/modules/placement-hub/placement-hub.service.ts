...
export class PlacementHubService {
  ...
  async fileHtml(slug: string, ticket?: string): Promise<{ html: Buffer | string; updated?: string }> {
    ...
    if (!viewer) {
      const gz = buf.length <= PlacementHubService.FREE_SERVE_MAX_BYTES
        ? zlib.gzipSync(buf, { level: 6 }).length
        : Number.MAX_SAFE_INTEGER; // 원시 초과면 압축 계산 없이 즉시 거부
      if (buf.length > PlacementHubService.FREE_SERVE_MAX_BYTES || gz > PlacementHubService.FREE_SERVE_MAX_GZIP) {
        this.logger.error(
          `무료 서빙 차단(질량 초과): ${slug} — ${buf.length}B / gzip ${gz === Number.MAX_SAFE_INTEGER ? '미측정' : gz}B`,
        );
        throw new ForbiddenException({
          code: 'HUB_FREE_OVERSIZE',
          message: '이 표는 무료 공개 규격을 초과합니다 — 관리자에게 문의하세요.',
        });
      }
