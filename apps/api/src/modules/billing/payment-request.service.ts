...
export class PaymentRequestService {
  ...
  async create(actor: AuthUser, dto: CreatePaymentRequestDto) {
    ...
    if (path === 'student_self') {
      ...
      if (!dto.studentId)
        throw new BadRequestException('대상 학생(studentId)이 필요합니다.');
    ...
    if (path === 'student_self') {
      ...
      await this.notify.notify(link?.guardian_id, 'payment_requested', {
        requestId: req.id,
        studentId,
        neededCredits: dto.neededCredits,
      });
    }
    return { ...req, path };
  }
  ...
  async respond(id: string, dto: RespondPaymentRequestDto, actor: AuthUser) {
    ...
    if (req.status !== 'open')
      throw new BadRequestException('이미 처리된 결제요청입니다.');

    // 권한: 본인(학생) 또는 연결된 보호자
    const isStudentOwner =
      actor.role === AccountRole.STUDENT && actor.id === req.student_id;
    const isLinkedGuardian = actor.role === AccountRole.GUARDIAN;
    if (isLinkedGuardian)
      await this.assertApprovedLink(actor.id, req.student_id);
    ...
    if (dto.action === 'reject') {
      ...
      if (upd.count !== 1)
        throw new ConflictException('이미 처리된 결제요청입니다.');
      // 결제요청 거절 → 학생 알림
      await this.notify.notify(req.student_id, 'payment_responded', {
        requestId: id,
        status: 'rejected',
      });
    ...
    if (!canPayRequest(actor.role))
      throw new ForbiddenException('결제할 수 없는 역할입니다.');
    ...
    await this.notify.notify(req.student_id, 'payment_responded', {
      requestId: id,
      status: 'done',
      chargedCredits: req.needed_credits,
    });
