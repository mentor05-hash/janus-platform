...
export type TaskInput = { title: string; category?: string; subject?: string | null; dueDate?: string | null };
...
export class TasksService {
  ...
  private async generateSuggestions(studentId: string) {
    ...
    const suggestions: { source_key: string; title: string; category: string; subject?: string | null; due_date?: Date | null; cta_href: string }[] = [];
    ...
    const now = new Date();
    const today = new Date(`${now.toLocaleDateString('en-CA', { timeZone: 'Asia/Seoul' })}T00:00:00.000Z`);
    const in30 = new Date(today.getTime() + 30 * 86400000);
    const events = await this.prisma.academic_event.findMany({
      where: {
        start_date: { gte: today, lte: in30 },
        OR: [{ center_id: null }, ...(sp?.center_id ? [{ center_id: sp.center_id }] : [])],
      },
    ...
    await this.prisma.student_task.createMany({
      data: suggestions.map((s) => ({ ...s, student_id: studentId, created_by: 'auto' })),
      skipDuplicates: true,
    });
  }
  ...
  private async owned(user: AuthUser, id: string) {
    ...
    if (t.student_id !== user.id) throw new ForbiddenException('본인 할 일만 변경할 수 있습니다.');
    return t;
  }

  /** 상태 변경(todo/done/dismissed). done 시 done_at 기록. */
  async setStatus(user: AuthUser, id: string, status: 'todo' | 'done' | 'dismissed') {
  ...
  async remove(user: AuthUser, id: string) {
    ...
    if (t.created_by !== 'self') {
      ...
      await this.prisma.student_task.update({ where: { id }, data: { status: 'dismissed' } });
  ...
  // ── 마감 리마인더(D-1 / 당일) ──
  private static readonly OFFSETS: { days: number; key: string; label: string }[] = [
  ...
  async reminderTick() {
    await withCronLock(this.cache, 'task-reminder', 300, async () => {
  ...
  async runReminders(now = new Date()): Promise<number> {
    ...
    for (const off of TasksService.OFFSETS) {
      ...
      const rows = await this.prisma.student_task.findMany({
        where: { status: 'todo', due_date: target, NOT: { reminded: { has: off.key } } },
      ...
      for (const t of rows) {
        ...
        await this.notify.notify(t.student_id, 'task_reminder', { taskId: t.id, title: t.title, when: off.label });
        sent++;
      }
    }
    return sent;
