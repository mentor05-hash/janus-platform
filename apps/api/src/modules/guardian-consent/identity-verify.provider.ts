...
export class IdentityVerifyProvider {
  ...
  async verify(input: IdentityVerifyInput): Promise<IdentityVerifyResult> {
    ...
    if (this.mode() === 'real') {
      ...
      this.logger.warn('IDENTITY_VERIFY_PROVIDER=real 이지만 실 연동 미구현 — 검증 실패 처리.');
      return { ok: false, name, refMasked: '****', provider: 'real', reason: 'provider_not_wired' };
    }
    // stub: 이름 + (휴대폰 또는 인증서 사용자) 형식만 확인.
    if (name.length < 2) {
      return { ok: false, name, refMasked: '****', provider: 'stub', reason: 'invalid_name' };
    }
    if (input.method === 'phone' && !(input.phone ?? '').replace(/[^0-9]/g, '')) {
      return { ok: false, name, refMasked: '****', provider: 'stub', reason: 'phone_required' };
    }
    return { ok: true, name, refMasked: `stub:${this.mask(input.phone ?? input.birth)}`, provider: 'stub' };
  }
}
