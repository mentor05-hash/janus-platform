...
type ReportBody = { covered: string[]; diagnosis: string; next_actions: string[]; demo?: boolean; source?: 'audio' | 'fallback' };
type StudentViewBody = { covered: string[]; reviewPoints: string[]; nextLearning: string[]; demo?: boolean };
type GuardianViewBody = { progress: string; recommendedActions: string[]; effort: string; demo?: boolean };
...
export class ConsultReportService {
  ...
  async listMine(user: AuthUser) {
    ...
    const rows = await this.prisma.$queryRaw<Array<{ booking_id: string; status: string; updated_at: Date; sent_at: Date | null; opened_at: Date | null; start_at: Date | null; student_name: string | null; category: string | null; demo: boolean | null; has_views: boolean }>>`
    ...
    return rows.map((r) => ({
      ...
      startAt: r.start_at, studentName: r.student_name, category: r.category, demo: r.demo === true, hasViews: r.has_views === true,
    }));
  }
  ...
  private async recordFunnel(event: 'view' | 'cta', cta: string, meta: Record<string, unknown>) {
    try { await this.prisma.funnel_event.create({ data: { page: 'consult_report', event, cta, meta: meta as object } }); }
    catch { /* 계측 실패는 삼킨다 */ }
  }
  ...
  async generateViews(user: AuthUser, bookingId: string) {
    const { isTeacher } = await this.assertBooking(user, bookingId);
    if (!isTeacher) throw new ForbiddenException('담당 선생님만 리포트를 생성할 수 있습니다.');
    const report = await this.prisma.consult_report.findUnique({ where: { booking_id: bookingId } });
    if (report?.status === 'sent') throw new ForbiddenException('이미 발송된 리포트는 재생성할 수 없습니다.');
    const src = await this.getSummarySource(bookingId);
    if (!src) throw new NotFoundException('요약 원천이 없습니다 — 녹음 요약 또는 상담 기록(핵심 요약)이 필요합니다.');
    const reportId = await this.ensureParentReport(bookingId, src);

    let views: import('../llm/llm.types').ConsultReportViewsResult;
    try {
      views = await this.llm.consultReportViews({ origin: src.origin, covered: src.covered, diagnosis: src.diagnosis, nextActions: src.nextActions, subject: src.subject });
    } catch (e) {
      this.logger.warn(`2뷰 생성 실패(booking=${bookingId}): ${(e as Error).message}`);
      throw e;
    }
  ...
  private async parentAndViews(bookingId: string) {
    const report = await this.prisma.consult_report.findUnique({ where: { booking_id: bookingId } });
    if (!report) return { report: null, views: [] as Array<{ audience: string; body: unknown; status: string; shared_at: Date | null; opened_at: Date | null }> };
    const views = await this.prisma.consult_report_view.findMany({ where: { report_id: report.id } });
    return { report, views };
  }
  ...
  async viewsDetail(user: AuthUser, bookingId: string) {
    ...
    if (isStudent && !isTeacher && student && !student.opened_at) {
      await this.prisma.consult_report_view.updateMany({ where: { report_id: report.id, audience: 'student', opened_at: null }, data: { opened_at: new Date() } });
      await this.recordFunnel('view', 'opened', { bookingId, audience: 'student' });
      await this.logAccess('report_view', report.id, user.id, 'read');
    }
    const body = (report.body ?? {}) as ReportBody;
    return {
      bookingId, status: report.status, source: body.source ?? (report.transcript_id ? 'audio' : 'fallback'),
      sentAt: report.sent_at, updatedAt: report.updated_at,
      student: student ? (student.body as StudentViewBody) : null,
      guardian: guardian ? (guardian.body as GuardianViewBody) : null,
      guardianShared: !!guardian?.shared_at, guardianSharedAt: guardian?.shared_at ?? null,
      demo: ((student?.body as StudentViewBody | undefined)?.demo === true) || ((guardian?.body as GuardianViewBody | undefined)?.demo === true),
    };
  }

  /** 뷰 편집(선생님) — 발송 전. audience 별 부분 수정. 수정 시 demo 마크 해제. */
  async updateView(user: AuthUser, bookingId: string, audience: Audience, patch: Partial<StudentViewBody & GuardianViewBody>) {
    const { isTeacher } = await this.assertBooking(user, bookingId);
    if (!isTeacher) throw new ForbiddenException('담당 선생님만 수정할 수 있습니다.');
    const { report } = await this.parentAndViews(bookingId);
    if (!report) throw new NotFoundException('리포트가 없습니다.');
    if (report.status === 'sent') throw new ForbiddenException('발송된 리포트는 수정할 수 없습니다.');
    const cur = await this.prisma.consult_report_view.findUnique({ where: { report_id_audience: { report_id: report.id, audience } } });
  ...
  async shareToGuardian(user: AuthUser, bookingId: string) {
    ...
    if (!isStudent) throw new ForbiddenException('학생 본인만 공유할 수 있습니다.');
    const { report } = await this.parentAndViews(bookingId);
  ...
  async listGuardianShared(guardian: AuthUser, studentId: string) {
    ...
    const rows = await this.prisma.$queryRaw<Array<{ booking_id: string; shared_at: Date | null; opened_at: Date | null; start_at: Date | null; teacher_name: string | null; category: string | null }>>`
      ...
      LIMIT 100`;
    return rows.map((r) => ({ bookingId: r.booking_id, sharedAt: r.shared_at, openedAt: r.opened_at, startAt: r.start_at, teacherName: r.teacher_name, category: r.category }));
  }

  /** 학부모 — 공유된 guardian 뷰 상세(첫 열람 스탬프 + 계측). */
  async guardianViewDetail(guardian: AuthUser, studentId: string, bookingId: string) {
    await this.assertGuardianOfStudent(guardian, studentId);
    const report = await this.prisma.consult_report.findUnique({ where: { booking_id: bookingId } });
    if (!report || report.status !== 'sent') throw new NotFoundException('리포트가 없습니다.');
    // 자녀 소유 확인 — 이 리포트가 해당 학생의 것인지.
    const b = await this.prisma.booking.findUnique({ where: { id: bookingId }, select: { student_id: true } });
    if (b?.student_id !== studentId) throw new ForbiddenException('연결된 자녀의 리포트가 아닙니다.');
    const gv = await this.prisma.consult_report_view.findUnique({ where: { report_id_audience: { report_id: report.id, audience: 'guardian' } } });
    if (!gv || !gv.shared_at) throw new NotFoundException('공유되지 않은 리포트입니다.');
    if (!gv.opened_at) {
      await this.prisma.consult_report_view.update({ where: { id: gv.id }, data: { opened_at: new Date() } });
      await this.recordFunnel('view', 'opened', { bookingId, audience: 'guardian' });
      await this.logAccess('report_view', report.id, guardian.id, 'read');
    }
    return { bookingId, sentAt: report.sent_at, ...(gv.body as GuardianViewBody) };
  }
}
