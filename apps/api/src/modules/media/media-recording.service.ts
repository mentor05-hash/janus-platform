...
export class MediaRecordingService {
  ...
  }; // sttRequiresGuardianConsent 는 본부 확정(2026-07-19) — 나머지 🟡
  ...
  private async flags() {
    const row = await this.prisma.system_setting.findUnique({
      where: { key: MediaRecordingService.FLAG_KEY },
    });
    return {
  ...
  private async guardianGranted(
    studentId: string,
  ): Promise<{ granted: boolean; grantedAt: Date | null }> {
    const g = await this.prisma.consent_grant.findUnique({
      where: {
        student_id_kind: { student_id: studentId, kind: 'recording_stt' },
      },
    });
  ...
  async guardianConsentStatus(guardian: AuthUser, studentId: string) {
    if (guardian.role !== AccountRole.GUARDIAN)
      throw new ForbiddenException('보호자만 조회할 수 있습니다.');
  ...
  async setGuardianConsent(
    ...
    if (guardian.role !== AccountRole.GUARDIAN)
      throw new ForbiddenException('보호자만 설정할 수 있습니다.');
    const link = await this.prisma.guardian_student_link.findFirst({
      where: {
        guardian_id: guardian.id,
        student_id: studentId,
  ...
  async consent(user: AuthUser, bookingId: string, recording: boolean) {
    const f = await this.flags();
    if (!f.enabled)
      throw new ForbiddenException('상담 녹음 기능이 비활성화되어 있습니다.');
    ...
    if (!recording) {
      ...
      if (r?.status === 'recording' && r.egress_id) {
        await this.media
          .stopRecording(`consult_${bookingId}`, r.egress_id)
          .catch((e) => this.logger.warn(`철회 중단 실패: ${e}`));
        void this.realtime?.systemMessage(
          bookingId,
          '⏹ 상담 녹음이 중단되었습니다(동의 철회).',
        );
      }
      await this.prisma.consult_recording.upsert({
        ...
        update: {
          status: 'aborted',
          ...(side === 'student'
            ? { consent_student_at: null }
            : { consent_teacher_at: null }),
        },
      });
    ...
    const r = await this.prisma.consult_recording.upsert({
      ...
      create: {
        ...
        consent_policy_version: f.policyVersion,
        ...(side === 'student'
          ? { consent_student_at: now }
          : { consent_teacher_at: now }),
      },
      update:
        side === 'student'
          ? { consent_student_at: now }
    ...
    try {
      const started = await this.media.startRecording(`consult_${bookingId}`, {
        pathPrefix: 'consult-audio',
      });
      ...
      await this.prisma.consult_recording.update({
        ...
        data: {
          status: 'recording',
          egress_id: started.recordingRef,
          expires_at: expiresAt,
          consent_guardian_at: guardian.grantedAt,
        },
      });
      void this.realtime?.systemMessage(
        bookingId,
        '🔴 상담 녹음이 시작되었습니다(양측 동의).',
      );
      return { status: 'recording', recordingStarted: true };
    } catch (e) {
      this.logger.warn(
        `egress 시작 실패 booking=${bookingId}: ${(e as Error).message}`,
      );
      await this.prisma.consult_recording.update({
        where: { booking_id: bookingId },
      ...
      return {
        ...
        note: '녹음 시작에 실패했어요(상담은 계속 진행됩니다).',
      };
    }
  }
  ...
  private async purgeObject(key: string) {
    try {
      ...
      await client.send(
        new DeleteObjectCommand({ Bucket: s3cfg.bucket, Key: key }),
      );
      this.logger.log(`철회 파기: ${key}`);
    } catch (e) {
      this.logger.error(
        `철회 파기 실패(수동 정리 필요): ${key} — ${(e as Error).message}`,
      );
    }
  }
}
