...
export class LiveKitMediaProvider implements MediaProvider {
  ...
  // ── 서버 API(twirp) ──
  private baseHttp() {
    return this.url.replace(/^ws/i, 'http');
  } // wss→https · ws→http
  ...
  private async twirp<T>(
    ...
    const res = await fetch(`${this.baseHttp()}/twirp/${service}`, {
      ...
      headers: {
        ...
        authorization: `Bearer ${this.apiToken(grant)}`,
      },
      body: JSON.stringify(body),
    });
    if (!res.ok)
      throw new Error(
        `livekit ${service} → ${res.status}: ${(await res.text().catch(() => '')).slice(0, 200)}`,
      );
  ...
  private egressOutput(
    ...
    if (!this.egressS3)
      throw new Error(
        'LiveKit 녹화 저장소(LIVEKIT_EGRESS_S3)가 설정되지 않았습니다.',
      );
    const s3 = JSON.parse(this.egressS3) as {
      access_key: string;
      secret: string;
      bucket: string;
      region?: string;
    ...
    return {
      file_outputs: [{ filepath: `${pathPrefix}/${roomRef}-{time}.mp4`, s3 }],
    };
  }
  ...
  async startRecording(
    ...
    const res = await this.twirp<{ egress_id: string }>(
      'livekit.Egress/StartRoomCompositeEgress',
      {
        room_name: roomRef,
        audio_only: true,
        ...this.egressOutput(roomRef, opts?.pathPrefix),
      },
  ...
  async closeRoom(roomRef: string): Promise<void> {
    await this.twirp(
      ...
      { room: roomRef },
      { video: { roomAdmin: true, room: roomRef } },
    );
  }
}
