...
export class ClaimController {
  ...
  createClass(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Body() dto: ClassUpsertDto) {
    return this.svc.createClass(user, academyId, dto);
  }

  @Patch(':academyId/classes/:classId')
  updateClass(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Param('classId') classId: string, @Body() dto: ClassUpsertDto) {
    return this.svc.updateClass(user, academyId, classId, dto);
  }

  @Delete(':academyId/classes/:classId')
  deleteClass(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Param('classId') classId: string) {
    return this.svc.deleteClass(user, academyId, classId);
  }

  // ── 버스 노선/정류장(승인 owner) ──
  @Post(':academyId/bus-routes')
  createRoute(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Body() dto: BusRouteUpsertDto) {
    return this.svc.createBusRoute(user, academyId, dto);
  }

  @Patch(':academyId/bus-routes/:routeId')
  updateRoute(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Param('routeId') routeId: string, @Body() dto: BusRouteUpsertDto) {
    return this.svc.updateBusRoute(user, academyId, routeId, dto);
  }

  @Delete(':academyId/bus-routes/:routeId')
  deleteRoute(@CurrentUser() user: AuthUser, @Param('academyId') academyId: string, @Param('routeId') routeId: string) {
...
export class ClaimAdminController {
  ...
  @Post(':id/review')
  review(@CurrentUser() admin: AuthUser, @Param('id') id: string, @Body() dto: ClaimReviewDto) {
