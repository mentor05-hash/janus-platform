...
export class AssignmentService {
  ...
  // ── 학생: 자동배정 신청/조회/취소 ─────────────────────────────
  async requestAutoAssign(
    user: AuthUser,
    dto: { consultType: ConsultType; mode?: ConsultMode; subType?: string },
  ) {
    if (user.role !== AccountRole.STUDENT)
      throw new ForbiddenException('학생만 자동배정을 신청할 수 있습니다.');
    const sp = await this.prisma.student_profile.findUnique({
      where: { account_id: user.id },
      select: { center_id: true, type_code: true },
    });
    if (!sp)
      throw new NotFoundException('학생 등록(프로필)이 완료되지 않았습니다.');
    ...
    const dup = await this.prisma.auto_assign_request.findFirst({
      where: {
        student_id: user.id,
        status: 'waiting',
        consult_type: ctPrisma as never,
      },
    });
    if (dup) return dup;
  ...
  myRequests(user: AuthUser) {
    return this.prisma.auto_assign_request.findMany({
  ...
  async cancelRequest(user: AuthUser, id: string) {
    const r = await this.prisma.auto_assign_request.findUnique({
      where: { id },
    });
    if (!r || r.student_id !== user.id)
  ...
  async dashboard(actor: AuthUser, now = new Date()) {
    ...
    // 1) 대기열 상태 카운트
    const queueGroups = await this.prisma.auto_assign_request.groupBy({
      by: ['status'],
      where: cWhere,
      _count: { _all: true },
    });
    const queue = { waiting: 0, assigned: 0, cancelled: 0 } as Record<
      string,
      number
    >;
    ...
    // 2) 대기 목록(오래된 순)
    const waitingRows = await this.prisma.auto_assign_request.findMany({
      where: { ...cWhere, status: 'waiting' },
    ...
    const waitingList = waitingRows.map((r) => ({
      id: r.id,
      studentName: names[r.student_id] ?? '학생',
      consultType: consultTypeFromPrisma(r.consult_type),
      mode: r.mode,
    ...
    const originGroups = await this.prisma.booking.groupBy({
      ...
      where: {
        ...cWhere,
        origin: { in: AssignmentService.ORIGINS },
        created_at: { gte: since },
      },
      _count: { _all: true },
    });
    const byOrigin = Object.fromEntries(
      AssignmentService.ORIGINS.map((o) => [o, 0]),
    );
    for (const g of originGroups)
      if (g.origin) byOrigin[g.origin] = g._count._all;
    ...
    const todayGroups = ftIds.length
      ...
      : [];
    const todayByTeacher = new Map(
      todayGroups.map((g) => [g.teacher_id, g._count._all]),
    );
    const teachers = fullTimers
      .map((t) => ({
    ...
    if (!centerId) {
      ...
      const waitByCenter = await this.prisma.auto_assign_request.groupBy({
        by: ['center_id'],
        where: { status: 'waiting' },
        _count: { _all: true },
      });
      const asgByCenter = await this.prisma.booking.groupBy({
        by: ['center_id'],
        where: {
          origin: { in: AssignmentService.ORIGINS },
          created_at: { gte: since },
      ...
      const ftByCenter = await this.prisma.teacher_profile.groupBy({
        by: ['center_id'],
        where: { employment_type: '전임' },
        _count: { _all: true },
      });
      const wMap = new Map(
        waitByCenter.map((g) => [g.center_id, g._count._all]),
      );
      const aMap = new Map(
        asgByCenter.map((g) => [g.center_id, g._count._all]),
      );
      ...
      byCenter = centers
        .map((c) => ({
          centerId: c.id,
          center: c.name,
          waiting: wMap.get(c.id) ?? 0,
          assigned7d: aMap.get(c.id) ?? 0,
          fullTimers: fMap.get(c.id) ?? 0,
        }))
  ...
  async runFill(
    now = new Date(),
  ): Promise<{ assigned: number; waiting: number; questionBlocks: number }> {
    ...
    for (const req of waiting) {
      ...
      const pool = teachers.filter(
        (t) => !req.center_id || t.center_id === req.center_id,
      );
      let placed = false;
      for (let d = 0; d < HORIZON_DAYS && !placed; d++) {
        ...
        for (const t of pool) {
          ...
          const res = await this.booking.createAssigned({
            ...
            teacherGrade: t.grade ?? TeacherGrade.B,
            consultType: consultKo,
            subType: req.sub_type,
            mode,
            dateStr,
            slotStart: slots[start].index,
            slotEnd: slots[start].index + need,
            charge: 'session',
            origin: '자동배정',
          });
          if (res.ok) {
            await this.prisma.auto_assign_request.update({
              ...
              data: {
                status: 'assigned',
                assigned_booking_id: res.bookingId,
                assigned_at: now,
              },
            });
            assigned++;
    ...
    const openQs = await this.prisma.qna_post.findMany({
      where: {
        status: 'open',
        scheduled_booking_id: null,
        assigned_teacher_id: { in: teacherIds },
      },
      orderBy: { created_at: 'asc' },
      take: 200,
    });
    for (const q of openQs) {
      ...
      const need = Math.max(
        1,
        Math.round(
          (await this.booking.questionMinutes(q.difficulty)) / SLOT_MIN,
        ),
      );
      let placed = false;
      for (let d = 0; d < HORIZON_DAYS && !placed; d++) {
        ...
        const res = await this.booking.createAssigned({
          ...
          charge: 'free',
          origin: '질문배정',
          content: `질문 답변: ${(q.body ?? '').slice(0, 80)}`,
        });
        if (res.ok) {
          await this.prisma.qna_post.update({
            where: { id: q.id },
            data: { scheduled_booking_id: res.bookingId },
          });
          questionBlocks++;
          placed = true;
        }
    ...
    this.logger.log(
      `전임 채우기: 자동배정 ${assigned}/${waiting.length}, 질문블록 ${questionBlocks}/${openQs.length}`,
    );
  ...
  async scheduledReverseScan() {
    await withCronLock(
      ...
      async () => {
        ...
        if (r.created)
          this.logger.log(`역상담 자동배정: ${r.created}건(스캔 ${r.scanned})`);
      },
      this.logger,
    );
  }
    ...
    for (const s of candidates) {
      ...
      const prior = await this.prisma.booking.count({
        where: {
          ...
          status: { in: ['confirmed', 'done'] as never },
        },
      });
      ...
      let t =
        s.homeroom_teacher_id && ftIds.has(s.homeroom_teacher_id)
      ...
      for (let d = 0; d < HORIZON_DAYS; d++) {
        ...
        const res = await this.booking.createAssigned({
          ...
          centerId: t.center_id,
          teacherGrade: t.grade ?? TeacherGrade.B,
          consultType: ConsultType.HOMEROOM,
          mode,
          dateStr,
          slotStart: slots[start].index,
          slotEnd: slots[start].index + need,
          charge: revPolicy.free ? 'free' : 'session',
          origin: '역상담자동',
          direction: 'reverse',
        });
        if (res.ok) {
