...
const T = {
  ...
  noshowThreshold: 2,
  rejectThreshold: 5,
  rankingWeightDown: 0.3,
};

describe('가중 제한(§5-7)', () => {
  it('임계 미만이면 제한 없음', () => {
    const r = evaluatePenalty(
      { cancelCount: 2, noshowCount: 1, rejectCount: 4 },
      T,
    );
  ...
  it('미설정(null) 임계는 제한에 영향 없음', () => {
    const r = evaluatePenalty(
      { cancelCount: 99, noshowCount: 99, rejectCount: 99 },
      {
        ...
        noshowThreshold: null,
        rejectThreshold: null,
        rankingWeightDown: null,
      },
    );
    expect(r.restricted).toBe(false);
  });
  ...
  it('restrictMinutes 경과 시 제한 자동 해제(일시 제한)', () => {
    ...
    const within = evaluatePenalty(stats, T, {
      restrictMinutes: 30,
      penaltySinceMs: since,
      nowMs: since + 10 * 60_000,
    });
    expect(within.restricted).toBe(true);
    expect(within.lifted).toBe(false);
    // 창 경과(30분 초과) → 해제
    const after = evaluatePenalty(stats, T, {
      restrictMinutes: 30,
      penaltySinceMs: since,
      nowMs: since + 31 * 60_000,
    });
