...
export const INTENT_CARDS: Record<GatewayIntent, GatewayCard[]> = {
  diagnosis: [
    {
      title: '배치표로 지금 위치 확인',
      desc: '성적 입력 → 안정·적정·소신·상향 4구간',
      service: 'diagnosis',
      to: '/placement',
    },
    {
      title: '격차 리포트',
      desc: '목표까지 부족분을 근거와 함께',
      service: 'diagnosis',
      to: '/placement/gap',
    },
    {
      title: '전략 상담으로 잇기',
      desc: '진단 근거 위에서 지원선 설계',
      service: 'consulting',
      to: '/consulting/apply',
    },
  ],
  ...
  consulting: [
    {
      title: '상담 신청',
      desc: '배치표·격차 근거 위 1:1 전략 상담',
      service: 'consulting',
      to: '/consulting/apply',
    },
    {
      title: '먼저 진단부터',
      desc: '상담 전 배치표로 현재 위치 확인',
      service: 'diagnosis',
      to: '/placement',
    },
  ],
  tutoring: [
    {
      title: '선생님 찾기',
      desc: '풀별 응답시간·만족도로 1:1 매칭',
      service: 'tutoring',
      to: '/student/search',
    },
    {
      title: '격차 확인 후 매칭',
      desc: '진단이 과목·단원을 좁혀줍니다',
      service: 'diagnosis',
      to: '/placement',
    },
  ],
  lecture: [
    {
      title: '커리큘럼에서 강의로',
      desc: '처방 카드에서 필요한 강의로 연결',
      service: 'lecture',
      to: '/services/lecture',
    },
    {
      title: '진단으로 우선순위 정하기',
      desc: '어떤 강의부터인지 격차가 답합니다',
      service: 'diagnosis',
      to: '/placement',
    },
  ],
  ...
  unknown: [
    {
      title: '성적으로 진단받기',
      desc: '배치표·격차 리포트 — 무료로 시작',
      service: 'diagnosis',
      to: '/placement',
    },
    {
      title: '질문 올리기',
      desc: '문제 사진 한 장이면 AI 초안 즉시',
      service: 'qna',
      to: '/student/qna',
    },
    {
...
export function normalizeLlmResult(
  raw: Partial<GatewayInterpretation> | null | undefined,
): GatewayInterpretation {
  const intent: GatewayIntent = GATEWAY_INTENTS.includes(
...
export function classifyGatewayLlmFailure(
  ...
  if (msg.includes('구성되지') || msg.includes('지원하지'))
    return 'unconfigured';
  return 'llm_error';
}
