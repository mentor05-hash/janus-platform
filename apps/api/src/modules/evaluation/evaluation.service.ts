...
export class EvaluationService {
  ...
  async myLists(student: AuthUser) {
    const rows = await this.prisma.teacher_list_entry.findMany({ where: { student_id: student.id } });
    return {
      fit: rows.filter((r) => r.list_kind === 'fit').map((r) => r.teacher_id),
      unfit: rows.filter((r) => r.list_kind === 'unfit').map((r) => r.teacher_id),
    };
  }

  async classify(student: AuthUser, dto: ClassifyDto) {
    const teacher = await this.prisma.teacher_profile.findUnique({ where: { account_id: dto.teacherId } });
    ...
    if (existing) {
      if (existing.list_kind === dto.listKind) return { teacherId: dto.teacherId, listKind: dto.listKind };
      // 한 명은 한쪽만(§5-9) — 반대 목록 전환은 먼저 제거 후 재분류
      throw new ConflictException('이미 반대 목록에 분류되어 있습니다. 먼저 제거하세요.');
    }
    ...
    const lp = sp?.center_id
      ? await this.prisma.limit_policy.findUnique({ where: { center_id: sp.center_id } })
      : null;
    const limit =
      dto.listKind === 'fit'
  ...
  async review(bookingId: string, dto: ReviewDto, student: AuthUser) {
    ...
    if (student.role !== AccountRole.STUDENT || booking.student_id !== student.id) {
