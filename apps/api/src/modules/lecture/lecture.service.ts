...
export class LectureService {
  ...
  async catalog(user: AuthUser, subject?: string, q?: string) {
    ...
    const enrolled = new Set(
      (
        await this.prisma.lecture_enrollment.findMany({
  ...
  async enroll(user: AuthUser, lectureId: string) {
    if (user.role !== AccountRole.STUDENT)
      throw new ForbiddenException('학생만 수강신청할 수 있습니다.');
    ...
    try {
      await this.prisma.lecture_enrollment.create({
        data: { lecture_id: lectureId, student_id: user.id },
      });
    } catch (e) {
      if (
        ...
        throw new ConflictException('이미 수강 중인 강좌입니다.');
      throw e;
    }
  ...
  async create(
    ...
    if (user.role !== AccountRole.TEACHER)
      throw new ForbiddenException('선생님만 강좌를 등록할 수 있습니다.');
  ...
  async teacherLectures(user: AuthUser) {
    ...
    const counts = ids.length
      ? await this.prisma.lecture_enrollment.groupBy({
          by: ['lecture_id'],
  ...
  async setActive(user: AuthUser, lectureId: string, active: boolean) {
    const l = await this.prisma.lecture.findUnique({
      where: { id: lectureId },
    });
    if (!l || l.teacher_id !== user.id)
      throw new ForbiddenException('본인 강좌만 수정할 수 있습니다.');
  ...
  async detail(user: AuthUser, lectureId: string) {
    ...
    const [enr, ragg, myReview] = await Promise.all([
      this.prisma.lecture_enrollment.findUnique({
        where: {
          lecture_id_student_id: { lecture_id: lectureId, student_id: user.id },
        },
      }),
      this.prisma.lecture_review.aggregate({
        where: { lecture_id: lectureId },
        _avg: { rating: true },
        _count: { _all: true },
      }),
      this.prisma.lecture_review.findUnique({
        where: {
          lecture_id_student_id: { lecture_id: lectureId, student_id: user.id },
        },
      }),
    ...
    return {
      ...
      rating: ragg._avg.rating ? Math.round(ragg._avg.rating * 10) / 10 : null,
  ...
  async reviews(lectureId: string) {
    const rows = await this.prisma.lecture_review.findMany({
      where: { lecture_id: lectureId },
      orderBy: { created_at: 'desc' },
      take: 50,
    });
  ...
  async review(
    ...
    if (user.role !== AccountRole.STUDENT)
      throw new ForbiddenException('학생만 후기를 남길 수 있습니다.');
    const enr = await this.prisma.lecture_enrollment.findUnique({
      where: {
        lecture_id_student_id: { lecture_id: lectureId, student_id: user.id },
      },
    });
    if (!enr)
      throw new ForbiddenException('수강한 강좌만 후기를 남길 수 있습니다.');
    if (rating < 1 || rating > 5)
      throw new BadRequestException('평점은 1~5 입니다.');
  ...
  async updateProgress(user: AuthUser, lectureId: string, progress: number) {
    ...
    await this.prisma.lecture_enrollment.update({
      where: { id: enr.id },
      data: { progress: p, last_at: new Date() },
    });
  ...
  async myEnrollments(user: AuthUser) {
    ...
    return rows.map((r) => ({
      ...
      summary: r.lecture.summary,
      level: r.lecture.level,
      minutes: r.lecture.minutes,
