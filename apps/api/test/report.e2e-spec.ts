...
describe('3.3 신고·차단·AI 검토 통합', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.student_profile.create({
      data: { account_id: STU_B, center_id: CENTER },
    });
  });
  ...
  it('신고 등록 → AI 검토 첨부 → 관리자 처리(상태머신)', async () => {
    const r: any = await reports.create(studentUser, {
      targetType: 'teacher',
      targetId: TEACHER,
      reason: '상담 중 폭언을 들었습니다',
    });
    reportIds.push(r.id);
    expect(r.aiFlagged).toBe(true); // '폭언' 키워드 → flagged
    expect(r.status).toBe('received');

    const reviewing: any = await reports.handle(
      r.id,
      { status: 'reviewing' },
      adminUser,
    );
    expect(reviewing.status).toBe('reviewing');
    const resolved: any = await reports.handle(
      r.id,
      { status: 'resolved', action: '경고 조치' },
      adminUser,
    );
    expect(resolved.status).toBe('resolved');
    // 종료 상태에서 재전이 불가
    await expect(
      reports.handle(r.id, { status: 'reviewing' }, adminUser),
    ).rejects.toThrow();
  });
  ...
  it('M2/M3: AI 결과 별도 보존 + 센터 스코프', async () => {
    ...
    await expect(
      reports.handle(r.id, { status: 'reviewing' }, otherAdmin),
    ).rejects.toThrow();

    // 처리 후에도 AI 결과 보존(action 과 분리)
    await reports.handle(
      r.id,
      { status: 'resolved', action: '경고 조치' },
      adminUser,
    );
