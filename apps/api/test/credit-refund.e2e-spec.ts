...
describe('M4 환원 버킷 통합', () => {
  ...
  it('혼합 소비(부여 10k+구매 10k) 후 취소 → 부여 lot·구매분 원복', async () => {
    ...
    await prisma.$transaction((tx) =>
      credit.consumeWithin(tx, STU, 20_000, { refType: 'booking', refId: REF }),
    );
    let acct = await prisma.credit_account.findUnique({
      where: { id: acctId },
    });
    expect(acct!.granted_balance).toBe(0);
    expect(acct!.purchased_balance).toBe(40_000);
    expect(
      (await prisma.weekly_credit_grant.findUnique({ where: { id: lotId } }))!
        .remaining,
    ).toBe(0);
