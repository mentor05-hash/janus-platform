...
describe('잔여 계약 경로', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.teacher_offline_availability.deleteMany({ where: { teacher_id: TEACHER } });
    await prisma.account.deleteMany({ where: { id: STU } });
    await prisma.account.create({ data: { id: STU, role: 'student' as any, center_id: CENTER, login_id: 'pay_h', pw_hash: 'x', name: 'p', status: 'approved' as any } });
    await prisma.student_profile.create({ data: { account_id: STU, center_id: CENTER } });
    await prisma.credit_account.create({ data: { student_id: STU, purchased_balance: 0, granted_balance: 0 } });
  });
  ...
  it('오프라인 가용 설정(본인) + 비소유 차단', async () => {
    ...
    const r: any = await availability.putOfflineAvailability(TEACHER, { enabled: true, timeWindows: [] }, teacherSelf);
    expect(r.enabled).toBe(true);
    const other: any = { id: STU, role: 'teacher' };
    await expect(availability.putOfflineAvailability(TEACHER, { enabled: false }, other)).rejects.toThrow();
  });
