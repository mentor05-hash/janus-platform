...
describe('janus_report 이력', () => {
  ...
  const rows = () => prisma.janus_report.findMany({ where: { student_id: student.id, kind: 'gap' }, orderBy: { created_at: 'desc' } });

  it('격차 리포트 산출 → 이력 1건 적재(payload=트렁크 봉투)', async () => {
    const r = await scores.gapReport(student, { mode: 'susi', ...target, myGrade: 2.5 });
  ...
  it('목표가 바뀌면 새 이력 적재', async () => {
    await scores.gapReport(student, { mode: 'susi', univ: '다른대', dept: '다른과', cut: 1.2, myGrade: 1.4 });
    const list = await rows();
  ...
  it('이력 적재 실패가 리포트 응답을 막지 않는다(프로필 없는 학생)', async () => {
    const ghost = { id: '00000000-0000-4000-8000-0000000000fe', role: 'student', centerId: student.centerId };
    const r = await scores.gapReport(ghost as any, { mode: 'susi', ...target, myGrade: 3.0 });
