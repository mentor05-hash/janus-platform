...
describe('b2 요금 가산(§5-2)', () => {
  ...
  beforeAll(async () => {
    ...
    await policy.updatePricing(
      { mode: 'offline', offlineOccupancyFee: 0 } as any,
      hq,
    );
    await policy.updatePricing(
      { mode: 'zoom', paidConsultingFee: 0 } as any,
      hq,
    );
  });
  ...
  it('오프라인 점유료가 견적에 가산된다', async () => {
    const base = await pricing.quoteSession(
      ConsultMode.OFFLINE,
      30,
      TeacherGrade.B,
      null,
    ...
    await policy.updatePricing(
      { mode: 'offline', offlineOccupancyFee: 5000 } as any,
      hq,
    );
    const withFee = await pricing.quoteSession(
      ConsultMode.OFFLINE,
      30,
      TeacherGrade.B,
      null,
  ...
  it('입시 유료컨설팅 단가가 입시 상담에만 가산된다', async () => {
    await policy.updatePricing(
      { mode: 'zoom', paidConsultingFee: 30000 } as any,
      hq,
    );
    const normal = await pricing.quoteSession(
      ConsultMode.ZOOM,
      30,
      TeacherGrade.B,
      null,
