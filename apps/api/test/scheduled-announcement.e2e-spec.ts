...
describe('예약 공지·사전알림·템플릿(§3)', () => {
  ...
  const cleanNotis = () =>
    prisma.notification.deleteMany({
      where: {
        recipient_id: { in: [STU, HQ.id] },
        type: { in: ['announcement', 'announcement_reminder'] },
      },
    });
  ...
  beforeAll(async () => {
    ...
    await prisma.scheduled_announcement.deleteMany({
      where: { created_by: HQ.id },
    });
    await prisma.announcement_template.deleteMany({
      where: { created_by: HQ.id },
    });
    await cleanNotis();
  });
  ...
  it('예약 등록 → 즉시 미발송(pending), 도래 후 runDue 로 발송', async () => {
    ...
    expect(
      await prisma.notification.count({
        where: {
          recipient_id: STU,
          type: 'announcement',
          payload: { path: ['title'], equals: '예약공지' },
        },
      }),
    ).toBe(0);
    ...
    await prisma.scheduled_announcement.update({
      ...
      data: { scheduled_at: new Date(Date.now() - 1000) },
    });
    ...
    expect(n).toBeTruthy();
    const row = await prisma.scheduled_announcement.findUnique({
      where: { id: r.scheduledId },
    });
  ...
  it('발송 전날(24h 내) → 예약자(작성자)에게 사전알림 1회', async () => {
    ...
    const noti = await prisma.notification.findFirst({
      where: {
        ...
        payload: { path: ['scheduledId'], equals: r.scheduledId },
      },
    });
    expect(noti).toBeTruthy();
    // 재실행해도 중복 없음
    const before = await prisma.notification.count({
      where: { recipient_id: HQ.id, type: 'announcement_reminder' },
    });
    ...
    expect(
      await prisma.notification.count({
        where: {
          recipient_id: STU,
          type: 'announcement',
          payload: { path: ['title'], equals: '취소공지' },
        },
      }),
    ).toBe(0);
  });
  ...
  it('템플릿 저장 → templateId 로 발송', async () => {
    const t = await svc.createTemplate(HQ, {
      name: '휴원안내',
      targets: ['student'],
      title: '템플릿제목',
      body: '템플릿본문',
    ...
    const r = await svc.send(HQ, { templateId: t.id });
    expect((r as any).sent).toBeGreaterThanOrEqual(1);
    const n = await prisma.notification.findFirst({
      where: {
        recipient_id: STU,
        type: 'announcement',
        payload: { path: ['title'], equals: '템플릿제목' },
      },
    });
    expect(n).toBeTruthy();
  });
