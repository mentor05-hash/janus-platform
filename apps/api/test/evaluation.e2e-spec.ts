...
describe('3.2 평가·분류·랭킹 통합', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.student_profile.create({
      data: { account_id: STU_E, center_id: CENTER },
    });
    await prisma.account.create({
      data: {
        id: TEA_E,
    ...
    await prisma.teacher_profile.create({
      data: { account_id: TEA_E, center_id: CENTER, grade: 'B' as any },
    });
  });
  ...
  it('분류: fit 추가 → 반대(unfit) 동시 분류 차단 → 제거 후 재분류(§5-9)', async () => {
    await evalService.classify(studentUser, {
      teacherId: TEA_E,
      listKind: 'fit',
    });
    ...
    await expect(
      evalService.classify(studentUser, {
        teacherId: TEA_E,
        listKind: 'unfit',
      }),
    ).rejects.toThrow();
  ...
  it('리뷰: 완료 상담 평가 → 평점·등급 재산정', async () => {
    ...
    const tp = await prisma.teacher_profile.findUnique({
      where: { account_id: TEA_E },
    });
    ...
    await expect(
      evalService.review(
        b.id,
        { ratingAttitude: 1, ratingContent: 1, ratingSkill: 1, ratingAgain: 1 },
        studentUser,
      ),
    ).rejects.toThrow();
  });
  ...
  it('M1: 동시 리뷰 lost-update 없음(트랜잭션+행잠금)', async () => {
    ...
    await Promise.all([
      evalService.review(
        b1,
        { ratingAttitude: 5, ratingContent: 5, ratingSkill: 5, ratingAgain: 5 },
        studentUser,
      ),
      evalService.review(
        b2,
    ...
    const expected =
      Math.round((overalls.reduce((a, c) => a + c, 0) / overalls.length) * 10) /
      10;
    const tp = await prisma.teacher_profile.findUnique({
      where: { account_id: TEA_E },
    });
