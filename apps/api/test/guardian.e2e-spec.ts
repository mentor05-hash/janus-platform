...
describe('2.2 학부모·공개정책·결제요청 통합', () => {
  ...
  it('연결 신청→학생 승인 상태머신', async () => {
    const link = await guardian.requestLink(guardianUser, {
      studentLoginId: 'student01',
      relation: '모',
    });
    expect(link.status).toBe('pending');
    const res = await guardian.respondLink(
      link.id,
      { action: 'approve' },
      studentUser,
    );
  ...
  it('보호자는 공개 항목만, 메모·비공개 노트 차단(§5-5)', async () => {
    const notes: any[] = await consultation.listForStudent(
      STUDENT,
      guardianUser,
    );
  ...
  it('결제요청 보호자 대납: 생성→pay→학생 크레딧 충전', async () => {
    ...
    const req: any = await paymentRequests.create(guardianUser, {
      neededCredits: 15_000,
      studentId: STUDENT,
    });
    expect(req.path).toBe('guardian_proxy');
    const res: any = await paymentRequests.respond(
      req.id,
      { action: 'pay' },
      guardianUser,
    );
