...
describe('학부모 열람 게이트(janus_report)', () => {
  ...
  beforeAll(async () => {
    ...
    await scores.gapReport(student, { mode: 'susi', univ: '게이트검증대', dept: '검증과', cut: 2.0, myGrade: 2.5 });
    await prisma.student_share_consent.deleteMany({ where: { student_id: s.id } });
    await prisma.guardian_data_consent.deleteMany({ where: { student_id: s.id, guardian_id: g.id } });
  });
  ...
  afterAll(async () => {
    ...
    await prisma.guardian_data_consent.deleteMany({ where: { student_id: student.id, guardian_id: guardian.id } });
    if (prevIsMinor !== null) {
      await prisma.user_consent.update({ where: { account_id: student.id }, data: { is_minor: prevIsMinor } });
    }
    await app.close();
  });
  ...
  describe('성인 학생 — 본인 동의 필요', () => {
    ...
    it('동의 없으면 열람 불가(기본 deny)', async () => {
      await expect(scores.listChildReports(guardian, student.id)).rejects.toThrow();
    });
    ...
    it('보호자의 전달동의만으로는 성인 학생 데이터를 열람할 수 없다', async () => {
      await prisma.guardian_data_consent.create({
        data: { guardian_id: guardian.id, student_id: student.id, verify_status: 'verified', verified_at: new Date(), consent_delivery: true, consent_at: new Date() },
      });
      await expect(scores.listChildReports(guardian, student.id)).rejects.toThrow(); // 여전히 학생 동의가 없다
      await prisma.guardian_data_consent.deleteMany({ where: { student_id: student.id, guardian_id: guardian.id } });
    });
  });
  ...
  describe('미성년 학생 — 보호자 본인확인 + 전달동의', () => {
    ...
    it('본인확인만 하고 전달동의가 없으면 열람 불가', async () => {
      await prisma.guardian_data_consent.create({
        data: { guardian_id: guardian.id, student_id: student.id, verify_status: 'verified', verified_at: new Date() },
      });
      await expect(scores.listChildReports(guardian, student.id)).rejects.toThrow();
    });
  ...
  it('승인되지 않은 연결은 차단(연결 없는 보호자)', async () => {
    ...
    const stranger = { id: '00000000-0000-4000-8000-0000000000fd', role: 'guardian', centerId: guardian.centerId };
    await expect(scores.listChildReports(stranger as any, student.id)).rejects.toThrow();
  });
