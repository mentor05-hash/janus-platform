...
describe('격차 리포트 대상 게이트(IDOR)', () => {
  ...
  beforeAll(async () => {
    ...
    const v = await prisma.account.findFirstOrThrow({
      where: { role: 'student', id: { not: a.id } },
      select: { id: true },
    });
    victimId = v.id;
    ...
    await prisma.janus_report.deleteMany({
      where: { student_id: { in: [a.id, victimId] } },
    });
  });

  afterAll(async () => {
    await prisma.janus_report.deleteMany({
      where: { student_id: { in: [attacker.id, victimId] } },
    });
    await app.close();
  });

  const victimRows = () =>
    prisma.janus_report.count({ where: { student_id: victimId } });

  it('학생이 타인 studentId 를 보내면 403 — 조용히 무시하지 않는다', async () => {
    await expect(
      scores.gapReport(attacker, { ...REQ, studentId: victimId }),
  ...
  it('타인 이력에 행이 쓰이지 않는다(쓰기 IDOR)', async () => {
    ...
    await scores
      .gapReport(attacker, { ...REQ, studentId: victimId })
      .catch(() => {});
  ...
  it('본인 id 를 명시해 보내는 것은 허용(정상 경로 유지)', async () => {
    const r = await scores.gapReport(attacker, {
      ...REQ,
      studentId: attacker.id,
    });
    expect(r.kind).toBe('gap');
    expect(
      await prisma.janus_report.count({ where: { student_id: attacker.id } }),
    ).toBeGreaterThan(0);
  });
  ...
  it('보호자는 승인 연결만으로는 부족 — O105 동의 게이트를 통과해야 한다', async () => {
    ...
    const listErr = await scores
      .listChildReports(guardian, link.student_id)
      .then(
        () => null,
        (e) => e,
      );
    const genErr = await scores
      .gapReport(guardian, { ...REQ, studentId: link.student_id })
      .then(
        () => null,
        (e) => e,
      );
  ...
  it('학생·보호자 외 역할은 403', async () => {
    ...
    await expect(scores.gapReport(teacher, REQ)).rejects.toThrow(
      /학생·학부모만/,
    );
  });
});
