...
describe('관리자 공지 알림(§3)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.notification.deleteMany({
      where: { type: 'announcement', recipient_id: { in: [STU, TEA, GUA] } },
    });
  });
  ...
  it('센터관리자 → 학생·선생님 공지, 학부모는 비대상이라 제외', async () => {
    const r = (await svc.send(centerAdmin, {
    ...
    const stu = await prisma.notification.findFirst({
      where: { recipient_id: STU, type: 'announcement' },
    });
    const tea = await prisma.notification.findFirst({
      where: { recipient_id: TEA, type: 'announcement' },
    });
