...
describe('실시간 게이트웨이(채팅·화이트보드)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!centerId) throw new Error('데모 학생·선생님에게 center_id 가 없다 — 시드를 확인하라.');
    ...
    prevRealtime = await prisma.system_setting.findUnique({ where: { key: 'realtime_features' } });
    ...
    await prisma.booking.deleteMany({ where: { student_id: sp.account_id, content: RT_CONTENT } });
    // 학생01↔선생님01 예약 1건(참여자 방). **상대시각** — 절대 날짜는 그 날짜가 지나면 조용히 부패한다.
    const startAt = new Date(Date.now() + 60 * 60 * 1000);
    const b = await prisma.booking.create({
      data: {
        student_id: sp.account_id, teacher_id: tp.account_id, center_id: centerId,
        consult_type: 'subject' as never, mode: 'chat' as never, direction: 'student' as never,
        status: 'confirmed' as never, charged_credits: 0,
        start_at: startAt, end_at: new Date(startAt.getTime() + 30 * 60 * 1000),
        content: RT_CONTENT,
      },
      select: { id: true },
    });
    ...
    studentToken = (await auth.login({ loginId: ACCOUNTS.student, password: DEMO_PW })).accessToken;
    teacherToken = (await auth.login({ loginId: ACCOUNTS.teacher, password: DEMO_PW })).accessToken;
  });
  ...
  afterAll(async () => {
    ...
    if (prevRealtime) {
      await prisma.system_setting.update({ where: { key: 'realtime_features' }, data: { value: prevRealtime.value as never } });
    } else {
      await prisma.system_setting.deleteMany({ where: { key: 'realtime_features' } });
    }
    await app.close();
  });
