...
describe('a3 구독 정기결제(autopay)', () => {
  ...
  afterAll(async () => {
    await prisma.payment.deleteMany({
      where: { pg_txn_id: `mock_${subId}:${dueAt.toISOString()}` },
    });
  ...
  it('도래 구독 청구 → 결제 기록 + 다음 결제일 이월', async () => {
    ...
    const sub = await prisma.student_subscription.findUnique({
      where: { id: subId },
    });
  ...
  it('마이크로초 정밀도 next_billing_at 도 청구된다', async () => {
    ...
    const beforeAt = (await prisma.student_subscription.findUnique({
      where: { id: sub2.id },
    }))!.next_billing_at!;

    await autopay.runDue(new Date());

    const afterAt = (await prisma.student_subscription.findUnique({
      where: { id: sub2.id },
    }))!.next_billing_at!;
    expect(afterAt.getTime()).toBeGreaterThan(beforeAt.getTime()); // 이월됨 = 청구 성공
    const pay = await prisma.payment.findFirst({
      where: { pg_txn_id: { startsWith: `mock_${sub2.id}:` } },
    });
    expect(pay?.status).toBe('done');
