...
async function main() {
  ...
  try {
    ...
    //    지급량은 **N23 확정값(O50, 2026-07-26 — D 등급사다리교정)**: 주 Standard 24k / 월 Premium 210k · VIP 380k.
    ...
    //    ⚠ 이 값은 정책값이다. 운영 중 변경은 seed 가 아니라 PATCH /hr/membership-grades/{id} 로 한다
    //      (seed 는 ON CONFLICT DO NOTHING 이라 기존 DB 에 반영되지 않는다 — 마이그레이션 0049 참조).
    ...
    //      가격은 N23 확정(O50)에서 승계값 그대로 유지 — 사다리 교정은 부여량으로만 했다.
